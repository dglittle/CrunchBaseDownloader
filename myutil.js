...
function sort(a, func) {
    a.sort(function (a, b) {
        return func(a) - func(b)
    })
}

function isAttached(ref) {
...
getCookies = function (cookie) {
    ...
    if (cookie) {
        foreach(cookie.split(/;/), function (c) {
            var m = c.match(/(.*?)=(.*)/)
...
function setCookie(name, value, expireTime) {
	if (expireTime)
		var expires = "; expires=" + new Date(expireTime).toGMTString()
	else
        var expires = ""
...
function drawHorzSplit(a, b) {
    var d = $('<table><tr valign="top"><td class="1"></td><td class="2"></td></tr></table>')
    d.find('.1').removeClass().append(a)
    d.find('.2').removeClass().append(b)
    return d
}
...
// ensure(obj, 'key', 'subKey', defaultValue)
ensure = function () {
...
deepMerge = function (objA, objB) {
    foreach(objB, function (v, k) {
        if (v == null) {
            delete objA[k]
        } else if (typeof(objA[k]) == "object" && typeof(v) == "object") {
            deepMerge(objA[k], v)
        } else {
            objA[k] = v
        }
...
randomIdentifier = function (size) {
    var firstChars = lowerChars + upperChars
